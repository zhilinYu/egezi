...
function loadList(getpage) {
    $.ajax({
        ...
        async: false,
        data: { 
        	merchant_account: $("input[name=merchant_account_search]").val(),   	
...
function getItemVal(obj){
	    ...
	    $('input[name=merchant_account]').attr("disabled",true);
	    $('input[name=job_member]').show();
	    $('input[name=job_member]').attr("disabled",true);
	    var tmpRole1 = 0;
